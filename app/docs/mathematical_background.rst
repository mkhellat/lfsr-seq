...
power-of-two reset strategy:

1. **Initialize**: Set tortoise :math:`T = S_0`, hare :math:`H = S_0 \cdot C`,
   power :math:`p = 1`, and period counter :math:`\lambda = 1`

2. **Iterate**:
   
   a. If :math:`T = H`, return period :math:`\lambda`
   b. If :math:`\lambda = p`:
   ...
   c. Move hare one step: :math:`H \leftarrow H \cdot C`
   d. Increment period counter: :math:`\lambda \leftarrow \lambda + 1`
   e. Repeat from step 2a

**Mathematical Correctness**:

We provide a rigorous proof that Brent's algorithm correctly identifies
the period :math:`\lambda` of a periodic state sequence.

.. prf:theorem:: Brent's Algorithm Correctness
   :label: theorem-brent-correctness

   Brent's cycle detection algorithm correctly identifies the period
   :math:`\lambda` of a periodic state sequence :math:`(S_0, S_1, S_2,
   \ldots)` generated by an LFSR.

.. prf:proof::

   Since the sequence is periodic with period :math:`\lambda`, there
   exists a smallest positive integer :math:`\lambda` such that
   :math:`S_\lambda = S_0` and :math:`S_{i+\lambda} = S_i` for all
   :math:`i \geq 0`.

   Let :math:`k` be the smallest integer such that :math:`2^k \geq
   \lambda`. The algorithm uses powers :math:`p_0 = 1, p_1 = 2, p_2 = 4,
   \ldots, p_k = 2^k`.

   **Key Observation**: When the period counter reaches :math:`p_i = 2^i`
   for some :math:`i < k`, the tortoise is reset to the hare's current
   position. At this point, the hare is at position :math:`S_{p_i}`. The
   tortoise remains at :math:`S_{p_i}` while the hare continues moving.

   **Case 1**: If :math:`\lambda \leq p_i`, then within the next
   :math:`p_i` steps, the hare will traverse positions :math:`S_{p_i+1},
   S_{p_i+2}, \ldots, S_{p_i+p_i} = S_{2p_i}`. Since :math:`\lambda \leq
   p_i`, we have :math:`S_{p_i+\lambda} = S_{p_i}` (by periodicity), so
   the tortoise and hare meet when the period counter reaches
   :math:`\lambda` after the reset.

   **Case 2**: If :math:`\lambda > p_i`, the algorithm continues until
   :math:`p_{i+1} = 2p_i`. Since the powers double, we eventually reach
   :math:`p_k` where :math:`2^k \geq \lambda`, at which point Case 1
   applies.

   **Termination**: The algorithm terminates when :math:`T = H`, which
   occurs when the hare has moved exactly :math:`\lambda` steps from the
   last tortoise reset position, yielding the period.
...
* **Space Complexity**: :math:`O(1)` (period-only mode) or
  ...
  - **Period-Only Mode**: The algorithm maintains two state pointers
    (tortoise and hare) and auxiliary variables (power, counter), each
    requiring :math:`O(d)` space for state vectors. For fixed :math:`d`,
    this is constant space :math:`O(1)`.
...
* **Operation Pattern**: Brent's uses a power-of-two reset strategy,
  while Floyd's uses constant-speed differential traversal (tortoise
  moves 1 step, hare moves 2 steps per iteration). Both achieve the same
  asymptotic :math:`O(\lambda)` complexity.

* **State Transition Count**: 
  
  - **Brent's**: At most :math:`3\lambda` state transitions in the
    worst case (hare moves :math:`\lambda` steps, plus resets)
  - **Floyd's**: Exactly :math:`3\lambda` state transitions (tortoise:
    :math:`\lambda`, hare: :math:`2\lambda`)
  - **Enumeration**: Exactly :math:`\lambda` state transitions
...
.. prf:example:: Brent's Cycle Detection
   ...
   * Initialize: :math:`T = S_0`, :math:`H = S_1`, :math:`p = 1`,
     :math:`\lambda = 1`
   * Check: :math:`T \neq H`, continue
   * Since :math:`\lambda = p = 1`, reset: :math:`T = S_1`, :math:`p = 2`,
     :math:`\lambda = 0`
   * Move hare: :math:`H = S_2`, :math:`\lambda = 1`; check: :math:`T \neq H`
   * Move hare: :math:`H = S_3`, :math:`\lambda = 2`; check: :math:`T \neq H`
   * Since :math:`\lambda = p = 2`, reset: :math:`T = S_3`, :math:`p = 4`,
     :math:`\lambda = 0`
   * Move hare: :math:`H = S_4`, :math:`\lambda = 1`; check: :math:`T \neq H`
   * Move hare: :math:`H = S_5`, :math:`\lambda = 2`; check: :math:`T \neq H`
   * Move hare: :math:`H = S_6`, :math:`\lambda = 3`; check: :math:`T \neq H`
   * Move hare: :math:`H = S_7`, :math:`\lambda = 4`; check: :math:`T \neq H`
   * Since :math:`\lambda = p = 4`, reset: :math:`T = S_7`, :math:`p = 8`,
     :math:`\lambda = 0`
   * Move hare: :math:`H = S_8`, :math:`\lambda = 1`; check: :math:`T \neq H`
   * Continue: :math:`H = S_9, S_{10}, \ldots, S_{14}`, :math:`\lambda = 7`; check: :math:`T \neq H`
   * Move hare: :math:`H = S_{15} = S_0`, :math:`\lambda = 8`; check: :math:`T \neq H`
   * Move hare: :math:`H = S_1`, :math:`\lambda = 9`; check: :math:`T \neq H`
   * Continue: :math:`H = S_2, S_3, \ldots, S_6`, :math:`\lambda = 14`; check: :math:`T \neq H`
   * Move hare: :math:`H = S_7`, :math:`\lambda = 15`; check: :math:`T = S_7 = H = S_7` ✓ MEET!
   
   The period is :math:`\lambda = 15`. Total state transitions: 15 (hare
   moves) + 3 (tortoise resets) = 18, which is within the :math:`3\lambda
   = 45` bound.
...
* **Time Complexity**: All three algorithms achieve :math:`O(\lambda)`
  ...
  factor (exactly 1×), Floyd's requiring exactly 3× state transitions,
  and Brent's requiring at most 3× state transitions.
