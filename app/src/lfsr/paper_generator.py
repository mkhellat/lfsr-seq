...
_LATEX_SPECIAL_CHARS_RE = re.compile("|".join(re.escape(c) for c in _LATEX_SPECIAL_CHARS))


def escape_latex(text: str) -> str:
    """
    Escape LaTeX-reserved characters in free-form, user-supplied text.

    Must be applied to any string that did not originate as
    already-valid LaTeX (e.g. titles, author names, and free-text
    observations), since those strings can contain ``%``/``_``/``&``/
    unbalanced braces that would otherwise break compilation or
    silently comment out part of the document.

    Replaces all reserved characters in a single pass (rather than
    sequential .replace() calls) so that replacement text -- e.g. the
    literal braces inside "\\textbackslash{}" -- is never re-escaped.
    """
    return _LATEX_SPECIAL_CHARS_RE.sub(lambda m: _LATEX_SPECIAL_CHARS[m.group()], text)
...
def generate_abstract_section(
    ...
    **Key Terminology**:
    ...
    abstract_text = "This paper presents a comprehensive analysis of Linear "
    abstract_text += "Feedback Shift Registers (LFSRs) over "
    ...
    if research_focus:
        abstract_text += f"The research focuses on {escape_latex(research_focus)}. "
    ...
    **Key Terminology**:
    ...
    if methods_used:
        ...
        for method in methods_used:
            latex_code.append(f"\\item {escape_latex(method)}")
    ...
    **Key Terminology**:
    ...
    if key_observations:
        ...
        for observation in key_observations:
            latex_code.append(f"\\item {escape_latex(observation)}")
        latex_code.append("\\end{itemize}")
        latex_code.append("")
...
def generate_complete_paper(
    ...
    **Key Terminology**:
    ...
    latex_code.append(f"\\title{{{escape_latex(title)}}}")
    if author:
        latex_code.append(f"\\author{{{escape_latex(author)}}}")
    else:
