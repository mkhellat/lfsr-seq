...
No SageMath dependency -- this module only touches stdlib
json/platform/sys/importlib.metadata. Focus is on: (1) JSON round-tripping
is exact (what goes in under analysis_config/analysis_results comes back
out byte-identical after a json.loads), (2) content actually varies when
input varies (e.g. two different configs produce different serialized
output, not some byte that never changes), and (3) file-writing helpers
write exactly what the string-returning versions would produce.
"""
...
class TestCaptureEnvironment:
    ...
    def test_packages_populated_when_available(self, monkeypatch):
        """Covers the try/for loop body: a fake
        importlib.metadata.version() that succeeds for 'sage' should
        populate env['packages']['sage'] with its version string."""
        ...
        # numpy/scipy raise PackageNotFoundError in this fake -> skipped.
    ...
    def test_distribution_not_found_for_individual_package_is_skipped(self, monkeypatch):
        ...
        try/except PackageNotFoundError -- one missing package must
        not prevent others from being captured."""
        from importlib import metadata as importlib_metadata
    ...
    def test_unexpected_exception_during_package_capture_is_swallowed(self, monkeypatch):
        """Covers the outer `except Exception: pass`: if something
        other than PackageNotFoundError blows up while iterating
        packages (e.g. version() itself raising a totally different
        error), capture_environment must not propagate it -- the whole
        packages capture is best-effort."""
        def fake_version(name):
