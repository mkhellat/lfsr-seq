...
- **Polynomial Ring**: A ring formed by polynomials with coefficients in a
  field. We work in polynomial rings over :math:`\text{GF}(2)` for binary LFSRs. A polynomial
...
1. :math:`I = \langle G \rangle` (:math:`G` generates the same ideal)
2. The solutions of the original system are the same as solutions of :math:`G`
...
- :math:`I` is the set of indices not in the cube
- :math:`p_S(I)` is the superpoly (depends only on variables in :math:`I`)
...
- Size of cubes (:math:`2^k` evaluations per cube of size :math:`k`)
- Number of cubes needed
- Degree of superpolies
- Keystream length required

For a cube of size :math:`k`, we need :math:`2^k` evaluations. If we need :math:`m` cubes and each
superpoly has degree :math:`d`, total complexity is roughly :math:`m \cdot 2^k \cdot 2^d`.
