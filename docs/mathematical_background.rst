...
**Finite Fields**:
* :math:`\mathbb{F}_q` denotes a finite field of order :math:`q = p^n` where :math:`p` is prime and :math:`n` is a positive integer
* :math:`\mathbb{F}_p` denotes a prime field (when :math:`n = 1`)
* :math:`\mathbb{F}_{p^n}` denotes an extension field (when :math:`n > 1`)

**Scalar Sequence**:
* :math:`s_0, s_1, s_2, \ldots \in \mathbb{F}_q` denotes the infinite sequence generated by the LFSR

**State Vectors**:
* :math:`S_i = (s_i, s_{i+1}, \ldots, s_{i+d-1}) \in \mathbb{F}_q^d` denotes the state vector at time :math:`i`
* The state vector is a window of :math:`d` consecutive elements from the scalar sequence
* The relationship is: :math:`S_i[j] = s_{i+j}` for :math:`j = 0, \ldots, d-1`

**LFSR Parameters**:
* :math:`d` denotes the degree (length) of the LFSR
* :math:`c_0, c_1, \ldots, c_{d-1} \in \mathbb{F}_q` denote the feedback coefficients
...
* Matrix multiplication uses row vectors: :math:`S_{i+1} = S_i \cdot C`

**Polynomials**:
* :math:`P(t)` denotes the characteristic polynomial of the state update matrix
* :math:`f(x)` or :math:`f(t)` denotes general polynomials (context-dependent)
...
2. **State vector**: :math:`S_i = (s_i, s_{i+1}, \ldots, s_{i+d-1}) \in \mathbb{F}_q^d`
   (a window of :math:`d` consecutive elements from the scalar sequence)

3. **Feedback coefficients**: :math:`c_0, c_1, \ldots, c_{d-1} \in \mathbb{F}_q`
...
The next state vector is computed by shifting all elements left and computing
...
:math:`\mathbb{F}_q` (where :math:`q = p^n` for prime :math:`p` and positive integer :math:`n`).
...
multiplication. Given state vector :math:`S_i = (s_i, s_{i+1}, \ldots, s_{i+d-1})`, 
the next state is:
...
companion matrix :math:`C` is a :math:`d \times d` matrix over :math:`\mathbb{F}_q`
...
* **Subdiagonal**: For :math:`i = 1, 2, \ldots, d-1`, the entry at position :math:`(i, i-1)` is 1
* **Last column** (column :math:`d-1`): Contains the LFSR feedback coefficients
  :math:`c_0, c_1, \ldots, c_{d-1}` at positions :math:`(i, d-1)` for
  :math:`i = 0, \ldots, d-1`
* **All other entries**: 0

**Critical Note**: Coefficients are stored in the **last column** (column :math:`d-1`),
not the last row. This is essential for parallel processing where coefficients
must be extracted for matrix reconstruction in worker processes using
:math:`c_i = C[i, d-1]` for :math:`i = 0, \ldots, d-1`.
...
The matrix :math:`C` correctly implements the shift operation (moving elements left)
and computes the new rightmost element using the linear feedback function.
...
where the last element is computed as :math:`s_4 = 1 \cdot 1 + 0 \cdot 1 + 0 \cdot 0 + 0 \cdot 0 = 1`.
...
**Note**: In fields where :math:`-1 \neq 1` (i.e., :math:`q > 2`), the formula uses
subtraction. In :math:`\mathbb{F}_2`, since :math:`-1 = 1`, the formula becomes:
...
**Example**: For :math:`C` with coefficients :math:`[1, 1, 0, 0]` over :math:`\mathbb{F}_2`:

Using the general formula :math:`P(t) = t^d - c_{d-1} t^{d-1} - \cdots - c_0`:

.. math::

   P(t) = t^4 - c_3 t^3 - c_2 t^2 - c_1 t - c_0 = t^4 - 0 \cdot t^3 - 0 \cdot t^2 - 1 \cdot t - 1

Since we are in :math:`\mathbb{F}_2` where :math:`-1 = 1`, this simplifies to:

.. math::

   P(t) = t^4 + t + 1

This polynomial is **primitive** and has order 15, giving maximum period.
...
* :math:`t^2 = t^2 \not\equiv 1`
* :math:`t^3 = t^3 \not\equiv 1`
* :math:`t^4 = t + 1 \not\equiv 1` (since :math:`t^4 = P(t) - (t + 1) = (t^4 + t + 1) - (t + 1)`)
...
**Example**: For the 4-bit LFSR with primitive polynomial :math:`P(t) = t^4 + t + 1`
...
of length 4. The exact coefficients depend on the sequence and initial state.
...
:math:`[1, 1, 0, 0]` over :math:`\mathbb{F}_2` (primitive polynomial case).
...
The coefficients :math:`c_0 = 1, c_1 = 1, c_2 = 0, c_3 = 0` are stored in the
last column at positions :math:`(0,3), (1,3), (2,3), (3,3)` respectively.
...
The sequence has period 15, which equals the matrix order. Since the polynomial
is primitive, all non-zero states have the maximum period of 15.

Step 6: Factorization
~~~~~~~~~~~~~~~~~~~~~

Since :math:`P(t) = t^4 + t + 1` is **irreducible** (and primitive) over
:math:`\mathbb{F}_2`, it does not factor into lower-degree polynomials.
The factorization is trivial: :math:`P(t) = t^4 + t + 1` itself.

For a reducible polynomial example, consider :math:`Q(t) = t^4 + t^3 + t + 1`:
...
* Order of :math:`Q(t)`: :math:`\text{lcm}(1, 3) = 3` (with appropriate handling of the square factor)

**Note**: The order calculation for polynomials with repeated factors requires
considering the multiplicities. For square-free polynomials, the order is the
LCM of the orders of the irreducible factors.
