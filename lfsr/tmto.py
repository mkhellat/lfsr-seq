...
class HellmanTable:
    ...
    **Hellman Table**
        A precomputed table storing chains of state transitions. Each chain
        starts with a random state and ends with a distinguished point. The
        table enables fast state recovery by looking up chains that might
        contain the target state.

    **Chain**
        A sequence of states connected by the state update function. Each
        chain has length t and represents t consecutive state transitions.
        Chains are the fundamental building blocks of Hellman tables.

    **Distinguished Point**
        A state with a special property (e.g., leading k bits are zero) used
        to mark chain endpoints. Distinguished points make chain endpoints easy
        to identify and store, reducing storage requirements.

    **Reduction Function**
        A function R that maps states to starting points (keys). The reduction
        function creates chains: S_0 → f(S_0) → R(f(S_0)) → f(R(f(S_0))) → ...
