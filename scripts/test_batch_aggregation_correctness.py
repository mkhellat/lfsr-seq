#!/usr/bin/env python3
"""
Test correctness of batch aggregation implementation (Phase 2.2).

This script verifies that batch aggregation doesn't affect correctness:
1. Results match sequential for different problem sizes
2. All batches are processed correctly
3. Sentinel handling works correctly
4. Metrics are still meaningful
"""
...
def test_correctness(coeffs, gf_order, desc, num_workers=4):
    """Test that batch aggregation doesn't affect correctness."""
    print(f"\n{'='*80}")
    ...
    # Dynamic with batch aggregation (Phase 2.2)
    print(f"\n2. Dynamic mode with batch aggregation ({num_workers} workers):")
    start = time.time()
...
def main():
    """Run correctness tests for different problem sizes."""
    print("="*80)
    print("BATCH AGGREGATION CORRECTNESS TEST (Phase 2.2)")
    print("="*80)
    ...
    print(f"\n{'='*80}")
    if all_passed:
        print("✅ ALL TESTS PASSED - Batch aggregation maintains correctness!")
    else:
